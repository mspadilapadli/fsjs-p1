//* release 6
class AppleTree {
    #deathAge;
    constructor(matureAge = 5, deathAge, healthStatus = true) {
        this.age = 0;
        this.height = 0;
        this.healthStatus = healthStatus;
        this.matureAge = matureAge;
        this.#deathAge = deathAge;
        this.fruits = [];
        this.harvested = { good: 0, bad: 0, count: 0 };
    }

    get deathAge() {
        return this.#deathAge;
    }

    set deathAge(value) {
        this.#deathAge = value;
    }

    grow() {
        if (this.age < this.#deathAge && this.healthStatus) {
            this.age++;
            this.height += Math.floor(Math.random() * 6) + 1;
            if (this.age >= this.#deathAge) this.healthStatus = false;
        }
    }

    produceFruits() {
        if (
            this.age >= this.matureAge &&
            this.healthStatus &&
            this.age <= this.#deathAge
        ) {
            // let randomProduces = Math.floor(Math.random() * 16) + 5;
            // for (let i = 0; i < randomProduces; i++) {
            //     this.fruits.push(new Apple());
            // }

            // *one line
            this.fruits.push(
                ...Array.from(
                    { length: Math.floor(Math.random() * 16) + 5 },
                    () => new Apple()
                )
            );
        }
    }

    harvest() {
        //* use forEach
        // this.harvested.count = this.fruits.length;
        // this.fruits.forEach((fruit) => {
        //     if (fruit.quality == "good") this.harvested.good++;
        //     else this.harvested.bad++;
        // });

        //* use reduce()
        this.harvested = this.fruits.reduce(
            (acc, fruit) => {
                fruit.quality == "good" ? acc.good++ : acc.bad++;
                acc.count++;
                return acc;
            },
            { good: 0, bad: 0, count: 0 }
        );

        //* use filter() - readable
        // this.harvested = {
        //     good: this.fruits.filter((fruit) => fruit.quality === "good")
        //         .length,
        //     bad: this.fruits.filter((fruit) => fruit.quality === "bad").length,
        //     count: this.fruits.length,
        // };
    }

    get harvestReport() {
        return `${this.harvested.count} (${this.harvested.good} ${
            Object.keys(this.harvested)[0]
        }, ${this.harvested.bad} ${Object.keys(this.harvested)[1]})`;
    }

    resetHarvest() {
        this.fruits = [];
        this.harvested = { good: 0, bad: 0, count: 0 };
    }
}

// const appleTree = new AppleTree();
// appleTree.age = 5;
// appleTree.deathAge = 10;

// console.log(appleTree);
// console.log(appleTree.deathAge);

// appleTree.grow();
// console.log(appleTree);

class Apple {
    constructor() {
        this.quality = this.randomQuality();
    }

    randomQuality() {
        let randomNum = Math.round(Math.random() * 3);
        return randomNum == 0 ? "bad" : "good";
    }
}

const appleZainudin = new Apple();
const appleHanyati = new Apple();

// console.log(appleZainudin);
// console.log(appleHanyati);
// appleTree.produceFruits();
// // console.log(appleTree);

// console.log(appleTree.harvested, "sebelum panen");
// appleTree.harvest(); // pohon di panen
// console.log(appleTree.fruits);
// console.log(appleTree.harvested, "setelah panen");
// console.log(appleTree.harvestReport);

// appleTree.resetHarvest();
// console.log(appleTree.fruits, "setlah panen");
// console.log(appleTree.harvested, " setelah reset");

//* : driver code
let appleTree = new AppleTree(1, 10); // menanam pohon
// console.log(appleTree);
// console.log(appleTree.deathAge);

do {
    appleTree.grow();
    appleTree.produceFruits();
    appleTree.harvest();
    console.log(
        `[Year ${appleTree.age} Report] Height = ${appleTree.height} m | Fruits harvested = ${appleTree.harvestReport}`
    );
    appleTree.resetHarvest();
} while (appleTree.healthStatus != false);

console.log(`The tree has met its end. :sad:`);
